    ...
    ui(new Ui::Info)
{
    ...
    connect(ui->button_recordData, SIGNAL(clicked()), this, SLOT(slot_buttonDatalog()));
}
...
void InfoWidget::slot_buttonDatalog()
{
    if (loggingData) {
        ui->label_recordData->setText("Data Saved");
        ui->button_recordData->setText("Record Data");
        loggingData = false;
        emit sgn_DatalogStop();
    }
    else {
        ui->label_recordData->setText("Saving Data to File ... ");
        ui->button_recordData->setText("Stop Recording");
        loggingData = true;
